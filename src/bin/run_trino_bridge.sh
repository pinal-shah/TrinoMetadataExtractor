...
LOGFILE="$ATLAS_LOG_DIR/atlas-trino-bridge.log"
...
JAVA_PROPERTIES="$ATLAS_OPTS -Datlas.log.dir=$ATLAS_LOG_DIR -Datlas.log.file=atlas-trino-bridge.log
...
do
  ...
  case "$option" in
    ...
    -s) IMPORT_ARGS="$IMPORT_ARGS -s $1"; shift;;
    -t) IMPORT_ARGS="$IMPORT_ARGS -t $1"; shift;;
    -cx) IMPORT_ARGS="$IMPORT_ARGS -cx $1"; shift;;
    -h) export HELP_OPTION="true"; IMPORT_ARGS="$IMPORT_ARGS -h";;
    --catalog) IMPORT_ARGS="$IMPORT_ARGS --catalog $1"; shift;;
    --table) IMPORT_ARGS="$IMPORT_ARGS --table $1"; shift;;
    --schema) IMPORT_ARGS="$IMPORT_ARGS --schema $1"; shift;;
    --cronExpression) IMPORT_ARGS="$IMPORT_ARGS --cronExpression $1"; shift;;
...
"${JAVA_BIN}" ${JAVA_PROPERTIES} -cp "${CP}" org.apache.atlas.trino.cli.TrinoExtractorMain $IMPORT_ARGS
